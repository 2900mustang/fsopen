...
describe.only('mostBlogs', () => {
	test('which author has it?', () => {
		const res = listHelper.mostBlogs(blogs)
		let obj = {
			'author': "Robert C. Martin",
			'blogs': 3
		}
		expect(res.author).toEqual(obj.author)
		expect(res.blogs).toEqual(obj.blogs)
	})
...
describe.only('most likes', () => {
	test('author whose blogs gained the most likes', () => {
		let obj = {
			author: "Edsger W. Dijkstra",
			likes: 17
		}
		...
		expect(res.author).toEqual(obj.author)
		expect(res.likes).toEqual(obj.likes)
	})
})